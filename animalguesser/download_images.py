...
(the in-* prefix makes iNaturalist images easy to spot for selective pruning).
"""
...
INAT_PER_ANIMAL = 6       # iNaturalist images to fetch per animal
...
async def process_animal(
    ...
    total_saved = wiki_saved + inat_saved
    note = "  *** NO IMAGES ***" if total_saved == 0 else ""
    print(
        f"[{idx:3d}/{total}] {animal:<32} "
        f"wiki={wiki_saved}  inat={inat_saved}  total={total_saved}{note}"
    )
...
async def main() -> None:
    ...
    print(f"  iNat quota: {INAT_PER_ANIMAL} per animal")
    print("-" * 60)
    ...
    print("Wikipedia photos have no prefix; iNaturalist photos are prefixed 'in-'.")
